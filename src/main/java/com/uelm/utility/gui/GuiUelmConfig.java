 ...
 * delays can be edited. Settings are written to config/uelmutility.cfg when
...
public class GuiUelmConfig extends GuiScreen {
    ...
    public void initGui() {
        ...
        replantDelayField = new GuiTextField(10, fontRendererObj, centerX - 100, top + 110, 95, 20);
        replantDelayField.setMaxStringLength(7);
        replantDelayField.setText(String.valueOf(ConfigHandler.replantDelay));

        recastDelayField = new GuiTextField(11, fontRendererObj, centerX + 5, top + 110, 95, 20);
        recastDelayField.setMaxStringLength(5);
        recastDelayField.setText(String.valueOf(ConfigHandler.recastDelayTicks));

        buttonList.add(new GuiButton(BTN_DONE, centerX - 100, top + 142, 200, 20, "Done"));
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(fontRendererObj, "Uelm Utility", centerX, top - 26, 0xFFFFFF);
        fontRendererObj.drawString("Replant Delay (" + unitName() + ")", centerX - 100, top + 99, 0xA0A0A0);
        fontRendererObj.drawString("Recast Delay (Ticks)", centerX + 5, top + 99, 0xA0A0A0);
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
        if (replantDelayField.isFocused() || recastDelayField.isFocused()) {
            // Only allow digits and editing keys in the number fields.
